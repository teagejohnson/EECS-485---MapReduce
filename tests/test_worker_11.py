...
def test_map_memory(mocker, tmp_path):
    ...
    with MemoryProfiler() as memory_profiler:
        ...
        # return the faked responses configured above.  When the student code
        # calls sys.exit(0), it triggers a SystemExit exception, which we'll
        # catch.
        try:
    ...
    # We expect one subprocess.Popen() call per input file, plus one for
    # sorting.
    assert count_popen_calls.call_count == 5
